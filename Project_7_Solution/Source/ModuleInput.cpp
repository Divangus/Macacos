...
update_status ModuleInput::PreUpdate()
{
	...
	if (pads->left_x < 0.0f) {
		keys[SDL_SCANCODE_A] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->left_x > 0.0f) {
		keys[SDL_SCANCODE_D] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->left_y > 0.0f) {
		keys[SDL_SCANCODE_S] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->left_y < 0.0f) {
		keys[SDL_SCANCODE_W] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->a==true) {
		keys[SDL_SCANCODE_SPACE] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->b == true) {
		keys[SDL_SCANCODE_K] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->y == true) {
		keys[SDL_SCANCODE_LSHIFT] = KEY_STATE::KEY_REPEAT;
	}
